...
def train(fold):
    ...
    df = pd.read_csv("/home/dragoshh1984/repos/kaggle/datasets/melanomia_classification/train_metadata.csv")
    ...
    train_aug = albumentations.Compose(
        [
            ...
            albumentations.PadIfNeeded(224, 224),
    ...
    train_metada = df_train.drop(["fold", "target", "image_name"], axis=1).values.tolist()
    train_targets = df_train.target.values

    valid_images = df_valid.image_name.values.tolist()
    valid_images = [os.path.join(training_data_path, i + ".png") for i in valid_images]
    valid_metadata = df_valid.drop(["fold", "target", "image_name"], axis=1).values.tolist()
...
def predict(fold):
    ...
    model_path = "/home/dragoshh1984/repos/kaggle/melanomia-classification"
    df_test = pd.read_csv("/home/dragoshh1984/repos/kaggle/datasets/melanomia_classification/test_metadata.csv")
    ...
    test_metadata = df_test.drop(["image_name", "target"], axis=1).values.tolist()
